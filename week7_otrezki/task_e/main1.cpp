...
int main()
{
    ...
    for (int i=0; i<n_queries; i++) {
        ...
        q.right--;
        queries.push_back(q);
        lsegments[q.left].push_back(i);
        rsegments[q.right].push_back(i);
    }

    multiset<int> el_mins;
    for (int i=0; i<n_elems; i++) {
        for (int j = 0; j < lsegments[i].size(); j++) {
            el_mins.insert(queries[lsegments[i][j]].el_min);
        }
        if (i != 0) {
            for (int j = 0; j < rsegments[i - 1].size(); j++) {
                auto it = el_mins.find(queries[rsegments[i - 1][j]].el_min);
                el_mins.erase(it);
            }
        }
        if (el_mins.size() != 0) {
