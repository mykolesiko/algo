    ...
    public:
        ...
        Polygon get_convex() {
            ...
            //for (int i = 0; i < vertices.size(); i++) {
                ...
                //Vector ex(p0, Point(p0.x + 1, p0.y));
                //double ang = ex.getAngle(vec1);
                //polar.push_back(make_pair(ang, i));
            ...
            for (int i = 2; i <= polar.size(); i++) {
                int ind = i % polar.size();
                Vector vec1(p1, p2);
                Point p3 = polar[ind];
                Vector vec2(p2, p3);
                long long dp = vec1 % vec2;
                while ((dp  <= 0) && (convex.size() > 1)) {
                    Point temp  = convex.back();
                    //cout << "esclude" << temp << " ";
                    convex.pop_back();
                    p2 = convex.back();
